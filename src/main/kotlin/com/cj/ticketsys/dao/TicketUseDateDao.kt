...
interface TicketUseDateDao {
    ...
    @Insert(
        "insert into ticket_usedate(name,remark,work_day,work_price,weekend_day,weekend_price,legal_day,legal_price,custom_dates,validity,not_dates) " +
                "values(#{name},#{remark},#{workDay},#{workPrice},#{weekendDay},#{weekendPrice},#{legalDay},#{legalPrice},#{customDates},#{validity},#{notDates})"
    )
    ...
    @Update(
        "update ticket_usedate set name=#{name},remark=#{remark},work_day=#{workDay},work_price=#{workPrice}," +
                ...
                "custom_dates=#{customDates},validity=#{validity},not_dates=#{notDates} " +
                "where id=#{id}"
    )
